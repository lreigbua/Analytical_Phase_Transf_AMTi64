...
# data = np.genfromtxt('../Final_Inputs/C3-B2/Temperature_Element_at_heigt_1.98.csv', delimiter=',')
# data = np.genfromtxt('../Final_Inputs/C4-B3/Temperature_Element_at_heigt_5.16.csv', delimiter=',')
# data = np.genfromtxt('../Final_Inputs/C5-B3/Temperature_Element_at_heigt_5.16.csv', delimiter=',')
# data = np.genfromtxt('../Final_Inputs/Cyl_t1s/Temperature_Element_at_heigt_53.04.csv', delimiter=',')
...
for i in range(1, len(t_input)):
    if t_input[i] - t_input[i-1] > min_t_step:
        ...
        min_t_step_c = (t_input[i]-t_input[i-1])/100 + 0.0001
  ...
  #First asses RLS state:  
    ...
    if RLS[i-1] == 1: # if it is solid:
        if T[i] >= T_melt:
            ...
            if dT/dt > -20.0 and dT/dt < 0 and beta_f[i-1] > beta_eq_i + 0.00001: # JMAK model beta to alpha
...
ax2.set_ylabel('Lath thickness (m)', color='r')
ax2.tick_params('y', colors='r')
ax2.set_ylim(0, 1.5)
ax1.legend(['Temperature'], loc='upper left')
ax2.legend(['Alpha lath thickness (m)', 'total_alpha_fraction'], loc='upper right')

fig.tight_layout()
plt.savefig('Lath_thickness.png')
...
ax2.set_ylim(0, 1)
ax1.legend(['Temperature'], loc='upper left')
ax2.legend(['Alpha predicted', 'Alpha measured XRD'], loc='right')
